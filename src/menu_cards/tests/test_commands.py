...
from seeder.management.commands.seed_db import EXAMPLE_DISHES, EXAMPLE_MEAT_DISHES, EXAMPLE_VEGAN_DISHES, EXAMPLE_VEGETARIAN_DISHES
...
class SeedDBCommandTest(TestCase):
    ...
    def test_mocked_cards_have_dishes(self):
        ...
        filled_cards = 3
        call_command('seed_db')
        filled_cards_from_db = MenuCard.objects.filter(dishes__isnull=False).distinct().count()
        empty_cards_from_db = MenuCard.objects.filter(dishes__isnull=True).distinct().count()
