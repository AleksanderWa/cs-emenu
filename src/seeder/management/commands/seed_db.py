...
EXAMPLE_MEAT_DISHES = ['Roasted Cactus', 'Spicy Herbed Chicken', 'Sugar Rib Roast',
                       'Sweet Curried Mussels', 'Steak with Winterberry Sauce', 'Flatbread', 'Meatballs']
...
def create_dishes():
    ...
    assign_dish_fields(vegetarian, EXAMPLE_VEGETARIAN_DISHES, FOOD_TYPE_CHOICES.vegetarian)
...
def create_menu_cards():
    meat_dishes = factory.Iterator(Dish.objects.filter(food_type=FOOD_TYPE_CHOICES.meat))
    meat_card = MenuCardsFactory(dish=meat_dishes)

    vegetarian_dishes = factory.Iterator(Dish.objects.filter(food_type=FOOD_TYPE_CHOICES.vegetarian))
    vegetarian_card = MenuCardsFactory(dish=vegetarian_dishes)

    vegan_dishes = factory.Iterator(Dish.objects.filter(food_type=FOOD_TYPE_CHOICES.vegan))
